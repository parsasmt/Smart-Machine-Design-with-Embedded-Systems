...
void untitled_step(void)
{
   ...
   */
  if (rtDW.is_active_c6_untitled == 0U) {
    rtDW.is_active_c6_untitled = 1U;

    /* Outport: '<Root>/barrier_alert' */
    rtY.barrier_alert = 0.0;
    rtDW.is_c6_untitled = IN_safe;
  } else if (rtDW.is_c6_untitled == IN_alarm) {
  ...
  if (rtDW.is_active_c7_untitled == 0U) {
    rtDW.is_active_c7_untitled = 1U;
    ...
    rtY.break_power = 0.0;
    rtDW.is_c7_untitled = IN_idel;
    rtDW.temporalCounter_i1 = 0U;
  } else {
    switch (rtDW.is_c7_untitled) {
     case IN_Change_speed:
      if (rtU.Encoder_input - rtDW.last_encode > 6.5) {
        rtDW.is_c7_untitled = IN_speed_rise;
      } else if (rtU.Encoder_input - rtDW.last_encode < -6.5) {
        rtDW.is_c7_untitled = IN_speed_fall;
      }
      break;
     ...
     case IN_alarm_on:
      if (rtU.Encoder_input < 10.0) {
        /* Outport: '<Root>/break_power' */
        rtY.break_power = 2.0;
        rtDW.is_c7_untitled = IN_low_break;
      } else if ((rtU.Encoder_input > 10.0) && (rtU.Encoder_input < 20.0)) {
        /* Outport: '<Root>/break_power' */
        rtY.break_power = 3.0;
        rtDW.is_c7_untitled = IN_mid_break;
      } else if (rtU.Encoder_input > 20.0) {
        /* Outport: '<Root>/break_power' */
        rtY.break_power = 4.0;
        rtDW.is_c7_untitled = IN_hard_break;
      }
      break;
     ...
     case IN_idel:
      {
        ...
        if ((rtDW.temporalCounter_i1 >= 15U) && (tmp <= 6.5) && (tmp >= -6.5)) {
          rtDW.is_c7_untitled = IN_speed_cal;
        } else {
          tmp = rtU.Encoder_input - rtDW.last_encode;
          if ((rtDW.temporalCounter_i1 >= 15U) && ((tmp > 6.5) || (tmp < -6.5)))
          {
            rtDW.is_c7_untitled = IN_Change_speed;
          } else if (rtY.barrier_alert == 1.0) {
            rtDW.is_c7_untitled = IN_alarm_on;
          }
        }
      }
      break;
   ...
   */
  if (rtDW.is_active_c1_untitled == 0U) {
    rtDW.is_active_c1_untitled = 1U;
    ...
    rtDW.is_c1_untitled = IN_Stop_car;
  } else {
    switch (rtDW.is_c1_untitled) {
     case IN_Start_move:
      if (rtU.MoveSensor == 0.0) {
        ...
        rtDW.is_c1_untitled = IN_straight_up;
      } else if (rtU.IR_sensor == 25.0) {
        /* Outport: '<Root>/car_position_y' incorporates:
         *  Outport: '<Root>/Car_speed'
         */
        rtY.car_position_y -= rtY.Car_speed;
        rtDW.is_c1_untitled = IN_straight_down;
      } else if (rtU.IR_sensor == 20.0) {
        /* Outport: '<Root>/car_position_x' incorporates:
         *  Outport: '<Root>/Car_speed'
         */
        rtY.car_position_x -= rtY.Car_speed;
        rtDW.is_c1_untitled = IN_straight_left;
      } else if (rtU.IR_sensor == 22.0) {
        /* Outport: '<Root>/car_position_x' incorporates:
         *  Outport: '<Root>/Car_speed'
         */
        rtY.car_position_x += rtY.Car_speed;
        rtDW.is_c1_untitled = IN_straight_right;
      }
      break;

     case IN_Stop_car:
      if (rtU.MoveSensor == 1.0) {
        rtDW.is_c1_untitled = IN_Start_move;
      }
      break;
   ...
   */
  if (rtDW.is_active_c2_untitled == 0U) {
    rtDW.is_active_c2_untitled = 1U;
    ...
    /* Outport: '<Root>/belt_alarm' */
    rtY.belt_alarm = 0.0;
    rtDW.is_c2_untitled = IN_idel_l;
  } else {
    switch (rtDW.is_c2_untitled) {
     case IN_b_alarm_off:
      rtDW.is_c2_untitled = IN_idel_l;
      break;
   ...
   */
  if (rtDW.is_active_c5_untitled == 0U) {
    rtDW.is_active_c5_untitled = 1U;

    /* Outport: '<Root>/battery_info' incorporates:
     *  Inport: '<Root>/battary_energy'
     */
    rtY.battery_info = rtU.battary_energy;
    rtDW.is_c5_untitled = IN_No_battery_reduse;
  } else if (rtDW.is_c5_untitled == IN_Battery_reduser) {
    if (rtU.MoveSensor == 0.0) {
      rtDW.is_c5_untitled = IN_No_battery_reduse;
    ...
    rtDW.timer = 0.0;
    rtDW.is_c5_untitled = IN_Battery_reduser;
  }
   ...
   *  Outport: '<Root>/battery_info'
   */
  if (rtDW.is_active_c4_untitled == 0U) {
    rtDW.is_active_c4_untitled = 1U;

    /* Outport: '<Root>/battery_alarm' */
    rtY.battery_alarm = 0.0;
    rtDW.is_c4_untitled = IN_not_low_battery;
  } else if (rtDW.is_c4_untitled == IN_low_battery) {
    ...
    rtDW.is_c4_untitled = IN_low_battery;
  }
   ...
   */
  if (rtDW.is_active_c3_untitled == 0U) {
    rtDW.is_active_c3_untitled = 1U;

    /* Outport: '<Root>/Light' */
    rtY.Light = 0.0;
    rtDW.is_c3_untitled = IN_light_off;
  } else if (rtDW.is_c3_untitled == IN_Light_on) {
    ...
    /* Outport: '<Root>/Light' */
    rtY.Light = 1.0;
    rtDW.is_c3_untitled = IN_Light_on;
  }
